...
describe("Exchange With Native", function () {
    beforeEach(async () => {
    ...
    const test = async (options) => {
        ...
        const selectorOne = web3.eth.abi.encodeFunctionSignature(
            "anyERC1155ForNative(bytes,address[7],uint8[2],uint256[6],bytes,bytes)"
        );
        const selectorTwo = web3.eth.abi.encodeFunctionSignature(
            "nativeForERC1155(bytes,address[7],uint8[2],uint256[6],bytes,bytes)"
        );

        const paramsOne = web3.eth.abi.encodeParameters(
            ["address[2]", "address", "uint256[3]", "string"],
            [
                [erc1155.address, ZERO_ADDRESS],
                payment.address,
